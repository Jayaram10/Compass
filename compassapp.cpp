...
void nearer(float x[2],float curr[2],float newer[2]){
...
public: node *root;
    ...
    void subtree_check(node *ptr,float x[2],float ans[2]){
        if(ptr!=NULL){
            nearer(x,ans,ptr->data);
            subtree_check(ptr->left,x,ans);
            subtree_check(ptr->right,x,ans);
        }
    }
    int acc;
    void NN(node *ptr,float x[2],float ans[2],int d=0){
        acc=accuracy;
...
void data_Read(char *fil,float latlon[30][2],string name[30],int &n){
    ifstream f(fil);
    string trash;
    while(!f.eof()){
        getline(f,name[n]);
...
int main(){
    kdtree PS(2),FS(2),HP(2);
    string lat,lon;
    float latlon[30][2],nn[2],a[2];string name[30];int n=0;
...
//actual application like display
    ...
    while(choice!=5){
        ...
        case 1: /*have to get string input and somehow convert it to our format of values*/
                ...
                convert(a,lat,lon);
                break;
        case 2: PS.NN(PS.root,a,nn);
                display(nn);
                break;
        case 3: FS.NN(FS.root,a,nn);
                display(nn);
                break;
        case 4: HP.NN(HP.root,a,nn);
                display(nn);
                break;
        default:break;
        }
